...
retention AS (
  SELECT
    -- Eligible users (only lifetime mode, otherwise counts = total users in range/custom)
    COUNTIF(${isLifetime ? `${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)` : 'TRUE'}) AS eligible_day1,
    COUNTIF(${isLifetime ? `${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 4 DAY)` : 'TRUE'}) AS eligible_day3,
    COUNTIF(${isLifetime ? `${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 8 DAY)` : 'TRUE'}) AS eligible_day7,
    COUNTIF(${isLifetime ? `${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 16 DAY)` : 'TRUE'}) AS eligible_day15,
    COUNTIF(${isLifetime ? `${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 31 DAY)` : 'TRUE'}) AS eligible_day30,
    COUNTIF(${isLifetime ? `${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 61 DAY)` : 'TRUE'}) AS eligible_day60,
    COUNTIF(${isLifetime ? `${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 91 DAY)` : 'TRUE'}) AS eligible_day90,

    -- Active users (eligibility only applied in lifetime mode)
    COUNTIF(JSON_QUERY(progress, '$.day1') IS NOT NULL ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY))` : ''}) AS day1_active,
    COUNTIF(JSON_QUERY(progress, '$.day3') IS NOT NULL ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 4 DAY))` : ''}) AS day3_active,
    COUNTIF(JSON_QUERY(progress, '$.day7') IS NOT NULL ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 8 DAY))` : ''}) AS day7_active,
    COUNTIF(JSON_QUERY(progress, '$.day15') IS NOT NULL ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 16 DAY))` : ''}) AS day15_active,
    COUNTIF(JSON_QUERY(progress, '$.day30') IS NOT NULL ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 31 DAY))` : ''}) AS day30_active,
    COUNTIF(JSON_QUERY(progress, '$.day60') IS NOT NULL ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 61 DAY))` : ''}) AS day60_active,
    COUNTIF(JSON_QUERY(progress, '$.day90') IS NOT NULL ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 91 DAY))` : ''}) AS day90_active,

    -- Completed exercises retention (eligibility only in lifetime)
    COUNTIF(
      ...
      ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY))` : ''}
    ...
    COUNTIF(
      ...
      ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 4 DAY))` : ''}
    ...
    COUNTIF(
      ...
      ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 8 DAY))` : ''}
    ...
    COUNTIF(
      ...
      ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 16 DAY))` : ''}
    ...
    COUNTIF(
      ...
      ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 31 DAY))` : ''}
    ...
    COUNTIF(
      ...
      ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 61 DAY))` : ''}
    ...
    COUNTIF(
      ...
      ${isLifetime ? `AND (${includeNullCreatedAt} DATE(created_at) <= DATE_SUB(CURRENT_DATE(), INTERVAL 91 DAY))` : ''}
